...
######### Examine reproduction amongst infected classes
...
surv_diff <- survdiff(Surv(time, status) ~ group, data = salty)
surv_diff
...
### look at cerc shedding
shed <- read.csv("cercs.anna.csv")
hist(shed$cercs, breaks = 20)

ggboxplot(shed, x = 'group', y = 'cercs', color = 'group')
fit_zipoisson <- glmmTMB(cercs ~ group + (1|snail), data=shed, ziformula=~1, family=poisson)
fit_hzip <- update(fit_zipoisson,
                   ziformula = ~.,
                   data = shed,
                   family = poisson(link="log"))
...
fit_zinbinom1 <- update(fit_hzip, family=nbinom1)
fit_hnbinom2 <- update(fit_hzip, family=list(family="truncated_nbinom1",link="log"))
AICtab(fit_zipoisson,fit_hzip, fit_zinbinom2, fit_hnbinom2)
summary(fit_zinbinom2)
Anova(fit_zinbinom2, type='III')
multzip <- emmeans(fit_zinbinom2, pairwise ~ group, adjust = 'mvt')
multzip

cercplot <- ggplot(shed, aes(x = gfct, y = cercs, fill = gfct)) + 
  geom_violin(trim = T, scale = 'width', draw_quantiles = c(0.25, 0.5, 0.75), adjust = 1.5) +
  labs(x="", y = "Cercariae count", hjust=10) +
  theme(text = element_text(size=30),
        axis.text.x = element_text(angle=90, hjust=1))

saltlabels <- c('0.38 ppt', '1.9 ppt', '3.8 ppt', '5.7 ppt')

cercplot + #geom_dotplot(binaxis='y', stackdir='centerwhole', stackratio = 0.5, dotsize = 0.5, position=position_dodge(1)) +
  scale_fill_manual(values=c("green4",'yellow3' ,"orangered" , 'blue4')) + theme_classic() +
  theme(text = element_text(size=10), legend.position='none') + scale_x_discrete(labels= saltlabels)
